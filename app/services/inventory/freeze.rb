...
module Inventory
  class Freeze
    def self.call!(sku:, reason: "manual", idempotency_key:, meta: {})
      new(sku:, reason:, idempotency_key:, meta:).call!
    end

    def initialize(sku:, reason:, idempotency_key:, meta:)
      @sku = sku
      @reason = reason.to_s.presence || "manual"
    ...
    def call!
      ...
      InventoryBalance.transaction do
        ...
        if balance.frozen_now? && balance.freeze_reason.to_s == @reason

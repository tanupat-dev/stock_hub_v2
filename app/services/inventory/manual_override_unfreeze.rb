  ...
  class ManualOverrideUnfreeze
    ...
    def call!
      ...
      InventoryBalance.transaction do
        ...
        # ✅ override: unfreeze regardless of reason (manual/system/anything)
        b.update!(frozen_at: nil, freeze_reason: nil)
        result = :override_unfrozen
        after = snapshot(b)
      end

      Rails.logger.info(
        {
          event: "inventory.manual_override_unfreeze",
          sku: @sku.code,
          result: result,
          meta: @meta,
          before: before,

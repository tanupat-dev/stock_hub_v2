...
module Shopee
  class ImportReturns
    ...
    def open_workbook(filepath)
      ...
      if header.start_with?("PK")
        tmp_xlsx = Rails.root.join("tmp", "imports", "detected_excelx_#{SecureRandom.hex(8)}.xlsx")
        FileUtils.mkdir_p(tmp_xlsx.dirname)
        FileUtils.cp(path, tmp_xlsx)
        Roo::Excelx.new(tmp_xlsx.to_s)
      else
        Roo::Excel.new(path)
      end
    end

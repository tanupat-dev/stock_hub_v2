...
module Shopee
  class FailedDeliveriesController < ApplicationController
    def import
      shop = find_shop!
      ...
      raise ArgumentError, "only .csv and .xlsx are supported" unless %w[.csv .xlsx].include?(ext)

      tmp_dir = Rails.root.join("tmp", "uploads", "shopee_failed_deliveries")
      ...
      result = ::Shopee::ImportFailedDeliveries.call!(
        shop: shop,
      ...
      render json: {
        ...
        grouped_failed_deliveries: result[:grouped_failed_deliveries],
        success_returns: result[:success_returns],
        failed_returns: result[:failed_returns],
      ...
      Rails.logger.error(
        {
          event: "shopee.failed_deliveries.import.fail",
